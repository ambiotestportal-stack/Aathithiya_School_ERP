...
import { Plus, Clock, CheckCircle, XCircle, Calendar, X, Users } from 'lucide-react';

const ParentLeaveModal = ({ isOpen, onClose, onSuccess, children }: any) => {
  const [formData, setFormData] = useState({ student: '', reason: '', startDate: '', endDate: '' });
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl shadow-2xl max-w-lg w-full relative z-10 overflow-hidden flex flex-col">
            ...
            <div className="p-6">
              <form id="leave-form" onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-gray-700 text-sm font-medium mb-1.5">Select Child</label>
                  <select value={formData.student} onChange={e => setFormData({...formData, student: e.target.value})} className="w-full px-4 py-2.5 rounded-xl border bg-gray-50 outline-none" required>
                    <option value="">-- Select Child --</option>
                    {children.map((c: any) => <option key={c._id} value={c._id}>{c.user?.name}</option>)}
                  </select>
                </div>
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      for (const child of myChildren) {
        const leaveRes = await api.get(`/api/leave?studentId=${child._id}`);
        allRequests = [...allRequests, ...leaveRes.data];
      }
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Leave Applications</h1>
          <p className="text-slate-500 mt-1">Submit and track leave requests for your children.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase">Child</th>
      ...
      <ParentLeaveModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={fetchRequests} children={children} />
    </div>
  );
}
