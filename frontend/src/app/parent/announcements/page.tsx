  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        ...
        const classIds = childrenRes.data.map((c: any) => c.enrolledClass?._id || c.enrolledClass).filter(Boolean);
        
        // Build query string
        let url = '/api/notices?targetAudience=Parents';
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        ...
        <p className="text-slate-500 mt-1">Important updates and circulars from the school.</p>
      </div>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="py-16 text-center bg-white rounded-2xl border border-slate-200 flex flex-col items-center">
            ...
            <p className="text-slate-500">There are currently no new notices for parents.</p>
          </div>
        ) : (
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-3">
                <span className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-xs font-bold">
                  {notice.targetAudience === 'All' ? 'Public Broadcast' : 'Parent Notice'}
                </span>
