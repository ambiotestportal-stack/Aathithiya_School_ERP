...
import { Calendar, Clock, BookOpen, Layers, LayoutGrid, CalendarDays, CheckCircle, Sparkles } from 'lucide-react';
...
export default function TeacherTimetablePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setIsVisible(settingsRes.data.showTimetableToStaff !== false);

        if (settingsRes.data.showTimetableToStaff !== false) {
          const res = await api.get(`/api/timetable/teacher/${user?.id}`);
  ...
  // Flatten all classes scheduled across all timetables
  const allScheduledClasses = timetable.flatMap(t => 
    (t.periods || []).map((per: any) => ({
      ...per,
      dayOfWeek: t.dayOfWeek,
      enrolledClass: t.enrolledClass
    }))
  );

  const todayClassesCount = allScheduledClasses.filter(c => c.dayOfWeek === todayName).length;
  const totalClassesCount = allScheduledClasses.length;

  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-slate-500">
        <div className="w-10 h-10 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin mb-4"></div>
        <p className="font-medium text-slate-600">Loading teaching schedule...</p>
      </div>
    );
  }
  ...
  if (!isVisible) {
    return (
      <div className="space-y-6 max-w-5xl mx-auto mt-12 text-center">
        ...
        <p className="text-slate-500">The timetable is currently hidden by the administration.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">My Teaching Schedule</h1>
          <p className="text-slate-500 mt-1">View your weekly teaching schedule in organized boxes across all assigned classes.</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Weekly Periods</p>
            <p className="text-xl font-bold text-slate-900">{totalClassesCount} Classes</p>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Today's Schedule ({todayName})</p>
        ...
        <div className="col-span-2 sm:col-span-1 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-slate-900 flex items-center gap-1">
              <CheckCircle className="w-4 h-4 text-emerald-500" /> Active Schedule
            </p>
          </div>
        </div>
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-4 bg-slate-50/70 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-slate-700">Weekly Timetable Matrix</span>
            </div>
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-sm text-left border-collapse min-w-[900px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {DAYS.map((day, idx) => {
                  const dayClasses = allScheduledClasses.filter(c => c.dayOfWeek === day);
                  ...
                  return (
                    <tr key={day} className={`hover:bg-slate-50/40 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/20'}`}>
                      ...
                      {/* Period Boxes */}
                      {allPeriods.map(p => {
                        const period = dayClasses.find(per => per.periodNumber === p);
                            ...
                            {period ? (
                              <div className={`flex flex-col justify-between p-3.5 ${colors?.bg} ${colors?.hover} rounded-2xl border ${colors?.border} h-full min-h-[110px] shadow-xs hover:shadow-md transition-all group`}>
                                <div>
                                  ...
                                  <div className="text-xs font-bold text-slate-600 bg-white/80 border border-slate-200/50 rounded-lg px-2 py-1 mb-2 inline-block">
                                    {formatClassName(period.enrolledClass?.name, period.enrolledClass?.section)}
                                  </div>
                                </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          ...
          <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200 flex overflow-x-auto gap-2">
            {DAYS.map(day => {
              ...
              const dayCount = allScheduledClasses.filter(c => c.dayOfWeek === day).length;
          ...
          {/* Day's Period Boxes */}
          {(() => {
            const dayClasses = allScheduledClasses
            ...
            if (dayClasses.length === 0) {
              return (
                <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-12 text-center text-slate-500">
                  ...
                  <h3 className="text-lg font-bold text-slate-700">No Teaching Periods</h3>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="pt-4 border-t border-slate-200/60 mt-3 flex items-center justify-between">
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Class Taught:</span>
                        <span className="text-sm font-bold text-slate-900 bg-white px-3 py-1 rounded-xl border border-slate-200 shadow-xs">
                          {formatClassName(period.enrolledClass?.name, period.enrolledClass?.section)}
                        </span>
                      </div>
                    </motion.div>
